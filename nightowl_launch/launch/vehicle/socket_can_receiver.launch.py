...
def generate_launch_description():
    ...
    addLaunchArguments("interface", "can0")
    addLaunchArguments("interval_sec", "0.01")
    addLaunchArguments("use_bus_time", "false")
    addLaunchArguments("auto_configure", "true")
    addLaunchArguments("auto_activate", "true")
...
def launchSetup(context, *args, **kwargs):
    socket_can_receiver_node = LifecycleNode(
        ...
        remappings=[
            ("from_can_bus", LaunchConfiguration('output_can_receive')),
        ],
        output='screen')
