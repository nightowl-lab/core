    ...
    : Node("imu_driver", options)
{
    ...
    this->frame_id_ = this->declare_parameter<std::string>("frame_id", std::string("world"));
    ...
    while (rclcpp::ok())
    {
        ...
        for(int i = 0; i < data_length - 1; ++i)
        {
            if(this->model_ == "100D2" && data_raw[i] == 0xA5 && data_raw[i+1] == 0x5A)
            {
                ...
                msg_imu.angular_velocity.x = d2f_gyro(data + 17);
                msg_imu.angular_velocity.y = -d2f_gyro(data + 15);
                msg_imu.angular_velocity.z = d2f_gyro(data + 19);
